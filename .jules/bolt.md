...
**Action:** Always fetch the `Set` object directly from Zustand instead of fetching the array and creating a new `Set` locally. Ensure dependent list filtering operations (like `FeedSection` and `ReelViewer`) accept `Set` properties to prevent re-creation.
