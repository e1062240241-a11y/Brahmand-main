...
## 2026-09-24 - Unbounded followers array stripping & O(1) batch edge lookups for Temple Service
**Learning:** Storing full `followers` array lists inside cached `temple` objects causes unbounded Redis cache bloat, memory spikes, and risks exceeding Firestore's 1MB document limit as temples accumulate thousands of followers at 1 lakh+ scale. Stripping `followers` arrays from cache objects and resolving user follow status via direct $O(1)$ batch queries against the dedicated `temple_follows` collection eliminates payload bloat while capping legacy document `followers` arrays at 100 items on follow writes.
**Action:** Updated `TempleService.get_temples`, `get_nearby_temples`, `get_temple`, `follow_temple`, and `unfollow_temple` in `backend/services/temple_service.py` to strip `followers` from cached payloads, perform batch lookups on `temple_follows`, and cap legacy array updates.
