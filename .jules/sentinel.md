...
## 2026-08-05 - [CRITICAL] Prevent Information Exposure in Global Error Handler
**Vulnerability:** The `global_exception_handler` in `backend/main.py` was catching all exceptions, extracting the stack trace using `traceback.format_exc()`, and returning both the raw exception message and the full stack trace directly to the client in the 500 error response.
**Learning:** Returning stack traces (`traceback.format_exc()`) and raw exception details in HTTP responses acts as an Information Exposure (CWE-209/CWE-200) vulnerability. It can leak sensitive system information—such as internal code paths, logic flows, and module names—directly to end-users or malicious actors probing the API.
**Prevention:** Always return a generic error message (e.g., "An internal server error occurred.") to the client. Securely log the actual exception details, including stack traces, internally on the server using `logger.error(..., exc_info=True)`.
