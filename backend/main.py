...
async def send_community_message(
    ...
    # Enforce community access and membership boundaries (optimizes DB fetch and verifies state/national level)
    # ⚡ Bolt: Unified community boundary check to prevent unverified sockets from spamming database reads.
    try:
        community_id = await verify_community_access(db, user, community_id, is_read_only=False)
    except ValueError as e:
        status_code = 404 if str(e) == "Community not found" else 403
...
async def get_community_messages(community_id: str, subgroup_type: str, limit: int = 25, before_timestamp: Optional[str] = None, token_data: dict = Depends(verify_token)):
    ...
    # Enforce community access and membership boundaries before fetching DB
...
async def handle_send_dm_socket(sid, data):
    ...
    try:
        ...
        # ⚡ Bolt: Prevent unverified socket pipeline spamming by enforcing member access validation directly on the stream.
        if chat and user_id not in chat.get('members', []):
             return {'status': 'error', 'message': 'Unauthorized stream access'}
