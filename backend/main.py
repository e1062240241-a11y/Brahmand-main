...
async def _create_post_document(
    ...
    try:
        ...
        await task_queue.enqueue(following_feed_service.fanout_post_to_followers, db, user_id, post_id, created_at_iso)
...
async def follow_user(user_id: str, token_data: dict = Depends(verify_token)):
    ...
    try:
        await task_queue.enqueue(following_feed_service.backfill_following_inbox, db, current_user_id, user_id, 10)
...
async def unfollow_user(user_id: str, token_data: dict = Depends(verify_token)):
    ...
    try:
        await task_queue.enqueue(following_feed_service.clean_following_inbox, db, current_user_id, user_id)
