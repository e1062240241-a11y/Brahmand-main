...
for (const assetId of registeredAssetIds) {
  ...
  if (!curatedKeys.has(physKey) && !seenPhys.has(physKey)) {
    seenPhys.add(physKey);
    uncuratedList.push({ physKey, sampleAssetId: assetId });
  }
}

console.log('Total Uncurated Entities remaining:', uncuratedList.length);
console.log('\n--- BATCH 3 TARGET CANDIDATES (First 20) ---');
uncuratedList.slice(0, 20).forEach((item, idx) => {
  console.log(`${idx + 1}. Key: "${item.physKey}" | Sample Asset: "${item.sampleAssetId}"`);
});
