...
const HELP_TYPES = ['Event Volunteer', 'Cleaning', 'Prasadam Distribution', 'Administrative', 'Other'];
...
export default function TempleHelpRequestScreen() {
  ...
  useEffect(() => {
    const timer = setTimeout(async () => {
      if (templeLocation.length >= 3 && !selectedLocation) {
  ...
  const openModal = (type: 'help' | 'volunteers' | 'contact') => {
    setModalType(type);
  ...
  const handleContinue = async () => {
    ...
    try {
      ...
      Alert.alert('Success', 'Temple help request posted!', [{ text: 'OK', onPress: () => router.push('/(tabs)/profile') }]);
    } catch (error: any) {
