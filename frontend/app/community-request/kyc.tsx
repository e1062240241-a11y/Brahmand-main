...
export default function CommunityKYCStatusScreen() {
  ...
  const status = (user as any)?.kyc_status || null;
  ...
  useEffect(() => {
    if (!loadingStatus) {
      ...
      if (status === 'pending' || status === 'manual_review') {
        router.replace('/community-request/kyc-success');
      }
    }
  }, [loadingStatus, user?.kyc_status, router]);
  ...
  const requestId = (user as any)?.kyc_request_no || ("BR-" + (user?.id ? user.id.slice(0, 6).toUpperCase() : "8921"));
    ...
    >
      <SafeAreaView style={{ flex: 1 }} edges={isReview ? ['top', 'bottom'] : ['top']}>
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity style={styles.refreshBtn} onPress={refreshKycStatus}>
                ...
                <View style={styles.nextStepsSection}>
                  ...
                  <View style={{ marginTop: 16 }}>
                    ...
                    <TimelineItem 
                      isCompleted={status === 'verified'}
                      isActive={false}
                      title="Community Requests Live"
                      description="You can now share requests with the community."
                      iconName={status === 'verified' ? 'checkmark' : 'people'}
                      showLine={false}
                    />
                  </View>
                </View>
                  ...
                  <View style={{ flexDirection: 'row', gap: 12, alignItems: 'flex-start' }}>
                    ...
                    <View style={{ flex: 1, gap: 4 }}>
                      ...
                      <Text style={styles.expectBoxDescription}>
                        {status === 'verified' 
                          ? 'Your profile is now verified! You can post and share requests with the community.' 
                          : 'Our team is currently verifying your details. You will receive a push notification once the process is complete.'}
                      </Text>
                    </View>
                  </View>
                </View>
              ...
              <>
                ...
                <View style={styles.whiteCard}>
                  ...
                  <TouchableOpacity 
                    ...
                    onPress={() => router.push({
                      pathname: '/community-request/kyc-submit',
