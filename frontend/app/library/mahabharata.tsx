...
const geeta3DImage = require('../../assets/images/tab-bar/books/Mahabharat.webp');
...
export default function MahabharataPage() {
  ...
  const handleToggleBookmark = () => {
    const title = PARVA_NAMES[currentChapter - 1] || `पर्व ${currentChapter}`;
  ...
  const handleScroll = (event: any) => {
    ...
    updateProgress({
      id: BOOK_ID,
      chapterName: PARVA_NAMES[currentChapter - 1] || `पर्व ${currentChapter}`,
  ...
  const fetchChapterData = async (chNum: number) => {
    ...
    try {
      ...
      console.error('Failed to fetch book parva:', error);
    } finally {
      setLoading(false);
    }
  };
            ...
            <View style={{
              ...
              <View style={[styles.stickyTopHeader, { 
                ...
                <View style={styles.stickyChapterTitle}>
                  <Text style={[styles.headerText, nightMode && styles.textNightLight]}>* महाभारत *</Text>
                  <Text style={[styles.headerText, nightMode && styles.textNightLight]}>* {PARVA_NAMES[currentChapter - 1]} *</Text>
                </View>
            ...
            <View style={styles.pageContent}>
              ...
              <View style={styles.chapterSubHeader}>
                <Text style={[styles.subHeaderText, nightMode && styles.textNight]}>
                  {verses.length > 0 ? `कुल श्लोक/अध्याय: ${convertToHindiNumerals(totalVerses || verses.length)}` : ''}
                </Text>
              </View>
              ...
              <View style={styles.bottomNavContainer}>
                {currentChapter > 1 ? (
                  <TouchableOpacity style={[styles.bottomNavBtn, nightMode && styles.bottomNavBtnNight]} onPress={() => handleChapterChange(currentChapter - 1)}>
                    ...
                    <Text style={[styles.bottomNavText, nightMode && styles.textNight]}>पिछला पर्व</Text>
                  </TouchableOpacity>
                ) : <View style={{ width: 100 }} />}
                
                {currentChapter < TOTAL_CHAPTERS ? (
                  <TouchableOpacity style={[styles.bottomNavBtn, nightMode && styles.bottomNavBtnNight]} onPress={() => handleChapterChange(currentChapter + 1)}>
                    <Text style={[styles.bottomNavText, nightMode && styles.textNight]}>अगला पर्व</Text>
