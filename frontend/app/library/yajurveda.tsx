...
const CHAPTER_TITLES = Array.from({ length: 40 }, (_, index) => `Chapter ${index + 1}`);

const STORAGE_LAST_READ_KEY = (chapter: number) => `yajurveda:last-read:chapter:${chapter}`;
const STORAGE_BOOKMARK_KEY = (chapter: number) => `yajurveda:bookmark:chapter:${chapter}`;
...
export default function YajurvedaReaderScreen() {
  ...
  const loadChapter = useCallback(async (chapterNumber: number, openDirectly: false | 'start' | 'end' = false) => {
    ...
    if (openDirectly) setPendingOpenChapter({ chapter: safeChapterNumber, edge: openDirectly });
    try {
      const incoming = await loadYajurvedaChapter(safeChapterNumber);
  ...
  return (
    <BookLayout
      title="Yajurveda"
      chapterTitle={CHAPTER_TITLES[currentPageChapter - 1] || 'Yajurveda'}
