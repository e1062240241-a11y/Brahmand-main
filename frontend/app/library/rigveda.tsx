...
const CHAPTER_TITLES = Array.from({ length: 10 }, (_, index) => `Mandala ${index + 1}`);

const STORAGE_LAST_READ_KEY = (chapter: number) => `rigveda:last-read:mandala:${chapter}`;
const STORAGE_BOOKMARK_KEY = (chapter: number) => `rigveda:bookmark:mandala:${chapter}`;
...
export default function RigvedaReaderScreen() {
  const layout = useBookLayout();
  const router = useRouter();
  const progresses = useLibraryStore(state => state.progresses);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [chapterNum, setChapterNum] = useState(() => progresses['rigveda']?.chapterNum || 1);
  ...
  const loadChapter = useCallback(async (chapterNumber: number, openDirectly: false | 'start' | 'end' = false) => {
    ...
    if (openDirectly) setPendingOpenChapter({ chapter: safeChapterNumber, edge: openDirectly });
    try {
      const incoming = await loadRigvedaChapter(safeChapterNumber);
      setChapters((prev) => ({ ...prev, [safeChapterNumber]: incoming }));
    } catch (err: any) {
      const detail = err?.response?.data?.detail;
      setError(typeof detail === 'string' ? detail : `Failed to load Mandala ${safeChapterNumber}`);
    } finally {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      if (!chapters[chapterNum]?.length) {
        setError(`Mandala ${chapterNum} is taking too long to load. Please check the connection and try again.`);
        setLoading(false);
      }
    }, 45000);
  ...
  const loadingChapterLabel = pendingOpenChapter?.chapter ? `Loading Mandala ${pendingOpenChapter.chapter}` : 'Loading Mandala 1';
  ...
  return (
    <BookLayout
      title="Rigveda"
      chapterTitle={CHAPTER_TITLES[currentPageChapter - 1] || 'Rigveda'}
      ...
      cover={{ title: 'RIGVEDA', subtitle: 'ऋग्वेद', imageSource: rigvedaCover }}
      bookId="rigveda"
      bookmarkPrefix="rigveda:bookmark:mandala:"
    />
  );
}
