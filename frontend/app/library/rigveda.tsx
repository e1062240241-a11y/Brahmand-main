...
const geeta3DImage = { uri: 'https://brahmandfeed23.b-cdn.net/assets/tab-bar/books/Rigveda.webp' };
...
export default function RigvedaPage() {
  ...
  const handleToggleBookmark = () => {
    const title = CHAPTER_TITLES[currentChapter] || `मण्डल ${currentChapter}`;
  ...
  const fetchChapterData = useCallback(async (chNum: number) => {
    setLoading(true);
    try {
      const loadedVerses = await loadRigvedaChapter(chNum);
      if (loadedVerses) {
            ...
            <View style={{
              ...
              <View style={[styles.stickyTopHeader, { 
                ...
                <View style={styles.stickyChapterTitle}>
                  <Text style={[styles.headerText, nightMode && styles.textNightLight]}>* ऋग्वेद *</Text>
                  <Text style={[styles.headerText, nightMode && styles.textNightLight]}>* {CHAPTER_TITLES[currentChapter]} *</Text>
                </View>
              ...
              <View style={styles.bottomNavContainer}>
                {currentChapter > 1 ? (
                  <TouchableOpacity style={[styles.bottomNavBtn, nightMode && styles.bottomNavBtnNight]} onPress={() => handleChapterChange(currentChapter - 1)}>
                    ...
                    <Text style={[styles.bottomNavText, nightMode && styles.textNight]}>पिछला मण्डल</Text>
                  </TouchableOpacity>
                ) : <View style={{ width: 100 }} />}
                
                {currentChapter < TOTAL_CHAPTERS ? (
                  <TouchableOpacity style={[styles.bottomNavBtn, nightMode && styles.bottomNavBtnNight]} onPress={() => handleChapterChange(currentChapter + 1)}>
                    <Text style={[styles.bottomNavText, nightMode && styles.textNight]}>अगला मण्डल</Text>
