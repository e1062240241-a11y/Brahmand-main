...
const CHAPTER_TITLES = [
  ...
  'Kishkindha Kand (किष्किन्धाकाण्ड)',
  'Sundar Kand (सुंदरकाण्ड)',
  'Lanka Kand (लंकाकाण्ड)',
  'Uttar Kand (उत्तरकाण्ड)',
];

const STORAGE_LAST_READ_KEY = (chapter: number) => `ramcharitmanas:last-read:kand:${chapter}`;
const STORAGE_BOOKMARK_KEY = (chapter: number) => `ramcharitmanas:bookmark:kand:${chapter}`;
  ...
  return (
    <>
      <View style={styles.verseContainer}>
        ...
        {transliteration ? (
          <Text style={[styles.transliterationText, { color: nightMode ? '#D8C7A8' : '#5B4729', fontSize: layout.transliterationSize, lineHeight: layout.transliterationLineHeight, marginTop: 12 }]}> 
            {transliteration}
          </Text>
        ) : null}
...
export default function RamcharitmanasReaderScreen() {
  const layout = useBookLayout();
  const router = useRouter();
  const progresses = useLibraryStore(state => state.progresses);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const [chapterNum, setChapterNum] = useState(() => progresses['ramcharitmanas']?.chapterNum || 1);
  ...
  const loadChapter = useCallback(async (chapterNumber: number, openDirectly: false | 'start' | 'end' = false) => {
    ...
    if (openDirectly) setPendingOpenChapter({ chapter: safeChapterNumber, edge: openDirectly });
    try {
      const incoming = await loadRamcharitmanasKand(safeChapterNumber);
  ...
  return (
    <BookLayout
      ...
      chapterTitle={CHAPTER_TITLES[currentPageChapter - 1] || 'Ramcharitmanas'}
      ...
      cover={{ title: 'RAMCHARITMANAS', subtitle: 'SHRIMAD', imageSource: ramcharitmanasCover }}
      bookId="ramcharitmanas"
      bookmarkPrefix="ramcharitmanas:bookmark:kaanda:"
    />
  );
}
