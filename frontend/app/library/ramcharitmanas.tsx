...
const geeta3DImage = { uri: 'https://brahmandfeed23.b-cdn.net/assets/tab-bar/books/ramcharit.webp' };
...
export default function RamcharitmanasPage() {
  ...
  const handleToggleBookmark = () => {
    const title = KAND_NAMES[currentChapter - 1] || `काण्ड ${currentChapter}`;
  ...
  const fetchChapterData = useCallback(async (chNum: number) => {
    setLoading(true);
    try {
      const loadedVerses = await loadRamcharitmanasKand(chNum);
      if (loadedVerses) {
            ...
            <View style={{
              ...
              <View style={[styles.stickyTopHeader, { 
                ...
                <View style={styles.stickyChapterTitle}>
                  <Text style={[styles.headerText, nightMode && styles.textNightLight]}>* रामचरितमानस *</Text>
                  <Text style={[styles.headerText, nightMode && styles.textNightLight]}>* {KAND_NAMES[currentChapter - 1]} *</Text>
                </View>
            ...
            <View style={styles.pageContent}>
              ...
              <View style={styles.chapterSubHeader}>
                <Text style={[styles.subHeaderText, nightMode && styles.textNight]}>
                  {verses.length > 0 ? `कुल श्लोक/चौपाई: ${convertToHindiNumerals(totalVerses || verses.length)}` : ''}
                </Text>
              </View>
