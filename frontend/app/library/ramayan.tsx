...
const CHAPTER_TITLES = [
  'Bala Kaanda (बालकाण्ड)',
  'Ayodhya Kaanda (अयोध्याकाण्ड)',
  'Aranya Kaanda (अरण्यकाण्ड)',
  'Kishkindha Kaanda (किष्किन्धाकाण्ड)',
  'Sundara Kaanda (सुंदरकाण्ड)',
  'Yuddha Kaanda (युद्धकाण्ड)',
  'Uttara Kaanda (उत्तरकाण्ड)',
];

const STORAGE_LAST_READ_KEY = (chapter: number) => `ramayan:last-read:kaanda:${chapter}`;
const STORAGE_BOOKMARK_KEY = (chapter: number) => `ramayan:bookmark:kaanda:${chapter}`;
...
export default function RamayanReaderScreen() {
  ...
  const loadChapter = useCallback(async (chapterNumber: number, openDirectly: false | 'start' | 'end' = false) => {
    ...
    if (openDirectly) setPendingOpenChapter({ chapter: safeChapterNumber, edge: openDirectly });
    try {
      const incoming = await loadRamayanChapter(safeChapterNumber);
  ...
  useEffect(() => {
    ...
    const chapterPageCount = Math.max(0, Math.ceil((chapterPages[pendingOpenChapter.chapter]?.length ?? 0) / 2));
    const openSpread = chapterStartSpreads[pendingOpenChapter.chapter] + (pendingOpenChapter.edge === 'end' ? Math.max(0, chapterPageCount - 1) : 0);
  ...
  return (
    <BookLayout
      title="Ramayan"
      chapterTitle={CHAPTER_TITLES[currentPageChapter - 1] || 'Ramayan'}
