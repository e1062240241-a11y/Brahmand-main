...
export default function ProfileScreen() {
  ...
  }[] = [
    {
      ...
      items: [
        ...
        },
        {
          ...
          icon: "ribbon",
          label: t("personalityVerification"),
          route: "/profile/personality-verification",
          color: "#D4AF37",
        },
        {
          id: "notifications",
        ...
        },
        {
          ...
          disabled: true,
          subLabel: t("language") === "hi" ? "जल्द आ रहा है" : "Coming soon",
          color: "#D97706",
        },
      ],
    },
  ...
  const fetchProfile = useCallback(
    async (showLoading = true) => {
      ...
      try {
        ...
        console.log(
          "[Profile] getUserProfile success:",
  ...
  const showImageSourcePicker = (field: "photo" | "cover_photo") => {
    ...
    Alert.alert(
      ...
      [
        {
          text: t("language") === "hi" ? "गैलरी" : "Gallery",
          onPress: () => pickProfileImage(field, "library"),
        },
        {
          text: t("language") === "hi" ? "कैमरा" : "Camera",
          onPress: () => pickProfileImage(field, "camera"),
        },
  ...
  const handleRemoveProfilePhoto = async () => {
    try {
      ...
      showToast(
        t("language") === "hi"
          ? "प्रोफ़ाइल फ़ोटो नहीं हटाई जा सकी"
    ...
    Alert.alert(
      ...
      [
        ...(hasPhoto
          ? [
              {
                text: t("language") === "hi" ? "फ़ोटो देखें" : "View photo",
        ...
        {
          text:
            t("language") === "hi" ? "गैलरी से चुनें" : "Choose from gallery",
          onPress: () => pickProfileImage("photo", "library"),
        },
        {
          text: t("language") === "hi" ? "फ़ोटो खींचें" : "Take photo",
          onPress: () => pickProfileImage("photo", "camera"),
        },
        ...(hasPhoto
          ? [
              {
                text: t("language") === "hi" ? "फ़ोटो हटाएं" : "Remove photo",
  ...
  const handleShareProfile = async () => {
    ...
    try {
      await Share.share({
        ...
        title:
          t("language") === "hi"
            ? `ब्रह्मांड पर ${displayName}`
            : `${displayName} on Brahmand`,
      });
    } catch (error: any) {
  ...
  const saveLocation = async () => {
    ...
    if (parts.length < 2) {
      Alert.alert(
        ...
        t("language") === "hi"
          ? "स्थान को शहर, राज्य के रूप में दर्ज करें"
          : "Enter location as City, State",
      );
      return;
    }
    setSavingProfileField(true);
    try {
      ...
      showToast(
        t("language") === "hi" ? "स्थान अपडेट हो गया" : "Location updated",
      );
    } catch {
      showToast(
        t("language") === "hi"
          ? "स्थान अपडेट करने में विफल"
  ...
  const loadPosts = useCallback(
    async (reset = false) => {
      console.log(
        "[Profile] loading posts for userId:",
        userId,
        "reset:",
        reset,
      );
      if (!userId || (postsLoading && !reset)) {
        console.log("[Profile] skip loadPosts:", {
          userId,
          postsLoading,
          reset,
        });
      ...
      try {
        console.log("[Profile] calling getUserPosts with:", {
          userId,
          LIMIT,
          currentOffset,
        });
        ...
        console.log(
          "[Profile] getUserPosts response payload:",
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem(`profile_posts_${userId}`)
      .then((cached) => {
        if (cached) {
  ...
  useEffect(() => {
    const subscription = DeviceEventEmitter.addListener(
      "post_uploaded",
      (newPost) => {
        // If we are looking at our own profile, aggressively insert the new post
        if (newPost && (newPost.user_id === userId || !userId)) {
          setPosts((prevPosts) => {
            // Avoid duplicates
  ...
  const handleMenuPress = (item: SettingItem) => {
    if (item.id === "culture") {
      ...
      setTimeout(() => {}, Platform.OS === "ios" ? 400 : 50);
      return;
    }
  ...
  const handleLogout = () => {
    ...
    Alert.alert(
      ...
      t("language") === "hi"
        ? "क्या आप सचमुच लॉगआउट करना चाहते हैं?"
        : "Are you sure you want to logout?",
      [
        { text: t("cancel"), style: "cancel" },
        {
          text: t("language") === "hi" ? "लॉगआउट" : "Logout",
          style: "destructive",
  ...
  const handleDeletePost = async (post: any) => {
    ...
    try {
      ...
      showToast(
        t("language") === "hi"
          ? "पोस्ट सफलतापूर्वक हटा दी गई"
          : "Post deleted successfully",
      );
    } catch (error) {
      console.warn("Failed to delete post:", error);
      setPosts((prev) =>
        prev.some((item) => item.id === postId) ? prev : [removedPost, ...prev],
      );
      setPostsCount((prev) => prev + 1);
      Alert.alert(
        t("language") === "hi"
          ? "पोस्ट हटाने में असमर्थ"
  ...
  const confirmDeletePost = (post: any) => {
    if (Platform.OS === "web") {
      if (
        window.confirm(
          t("language") === "hi" ? "इस पोस्ट को हटाएं?" : "Delete this post?",
        )
      ) {
    ...
    Alert.alert(
      ...
      t("language") === "hi"
        ? "क्या आप सचमुच इस पोस्ट को हटाना चाहते हैं?"
  ...
  const savePostEdit = async () => {
    ...
    try {
      ...
      const updatedPost = response.data?.post
        ? response.data.post
        : { ...selectedPost, caption: editedCaption };
      setSelectedPost(updatedPost);
      setPosts((prev) =>
        prev.map((item) => (item.id === postId ? updatedPost : item)),
      );
  ...
  const handleLikePost = useCallback(
    async (post: any) => {
      ...
      setPosts((prev) =>
        prev.map((item) => (item.id === postId ? optimisticPost : item)),
      );
      ...
      try {
        ...
        if (updatedPost) {
          if (selectedPost?.id === postId)
            setSelectedPost((prev: any) => ({ ...prev, ...updatedPost }));
          setPosts((prev) =>
            prev.map((item) =>
              item.id === postId ? { ...item, ...updatedPost } : item,
            ),
          );
        }
      } catch (error) {
        ...
        setPosts((prev) =>
          prev.map((item) => (item.id === postId ? post : item)),
        );
      }
    },
    [selectedPost],
  ...
  const handleSubmitComment = async () => {
    if (!selectedCommentPost?.id || !commentText.trim() || commentSubmitting)
      return;
    ...
    try {
      ...
      if (updatedPost) {
        if (selectedPost?.id === selectedCommentPost.id)
          setSelectedPost((prev: any) => ({ ...prev, ...updatedPost }));
        setPosts((prev) =>
          prev.map((item) =>
            item.id === selectedCommentPost.id
              ? { ...item, ...updatedPost }
              : item,
          ),
        );
        setSelectedCommentPost((prev: any) =>
          prev?.id === selectedCommentPost.id
            ? { ...prev, ...updatedPost }
            : prev,
        );
      }
  ...
  const handleDeleteComment = async (comment: any) => {
    ...
    setPosts((prev) =>
      prev.map((p) => {
        if (p.id === targetPostId) {
          const currentTop = Array.isArray(p.top_comments)
            ? p.top_comments
            : [];
          return {
            ...p,
    ...
    if (selectedPost?.id === targetPostId) {
      setSelectedPost((prev: any) => {
        if (prev) {
          const currentTop = Array.isArray(prev.top_comments)
            ? prev.top_comments
            : [];
          return {
            ...prev,
    ...
    try {
      ...
      if (updatedPostFromServer) {
        setPosts((prev) =>
          prev.map((p) => {
            if (p.id === targetPostId) {
              const currentTop = Array.isArray(
                updatedPostFromServer.top_comments,
              )
                ? updatedPostFromServer.top_comments
                : [];
              return {
        ...
        setSelectedCommentPost((prev: any) => {
          if (prev?.id === targetPostId) {
            const currentTop = Array.isArray(updatedPostFromServer.top_comments)
              ? updatedPostFromServer.top_comments
              : [];
            return {
              ...prev,
      ...
      setPosts((prev) =>
        prev.map((p) => (p.id === targetPostId ? originalPost : p)),
      );
  ...
  const handleShareExternal = async (post: any) => {
    const appLink = post?.id
      ? `https://brahmand.app/post/${post.id}`
      : "https://brahmand.app/";
    ...
    try {
      if (
        ...
        FileSystemModule?.downloadAsync &&
        mediaUrl
      ) {
        ...
        const downloadRes = await FileSystemModule.downloadAsync(
          mediaUrl,
          localPath,
        );
        if (downloadRes?.uri) {
  ...
  const renderPost = ({ item }: { item: any }) => {
    const rawUrl =
      item.media_url || item.mediaUrl || item.image_url || item.image || "";
    const isVideo =
    ...
    const displayUrl =
      ...
      rawUrl;
    const isGallery =
      !isVideo && (item.media_count > 1 || item.is_carousel || item.carousel);
    ...
    <View style={{ flex: 1, backgroundColor: "#000000" }}>
      ...
      <View style={styles.container}>
        ...
        <Animated.View
          ...
          style={[
            styles.stickyNav,
            { paddingTop: insets.top, height: insets.top + NAV_BAR_HEIGHT },
          ]}
        >
          <TouchableOpacity
          ...
          <Animated.View
            style={[styles.navCenter, { opacity: navTitleOpacity }]}
          >
        ...
        <Animated.FlatList
          ...
          keyExtractor={(item, index) =>
            item.id ? `post-${item.id}` : `post-idx-${index}`
          }
          numColumns={3}
        ...
        <Modal visible={showSettingsModal} animationType="slide" transparent>
          <View style={styles.modalOverlay}>
            ...
            />
            <View
              style={[styles.settingsSheet, { paddingBottom: insets.bottom }]}
            >
                  ...
                  if (
                    ...
                    if (idx >= 0) {
                      setTimeout(() => {
                        ...
                        setActivePostKey(
                          String(
                            selectedPost.id || selectedPost.media_url || 0,
                          ),
                        );
                      }, 200);
                    }
                ...
                >
                  ...
                  <View style={styles.sheetHeader}>
                    <Text style={styles.sheetTitle}>
                      {t("language") === "hi" ? "टिप्पणियाँ" : "Comments"} (
                      {selectedCommentPost?.comments_count ??
                        postComments.length ??
                        0}
                      )
                    </Text>
                  ...
                  {commentsLoading ? (
                    ...
                    (() => {
                      ...
                      const repliesMap = postComments.reduce(
                        (acc, c) => {
                          if (c.parent_id) {
                        ...
                        <FlatList
                          ...
                          renderItem={({ item }) => {
                            const canDelete =
                              ...
                              selectedCommentPost?.user_id === user?.id;
                  ...
                  >
                    ...
                    <MentionInput
                      ...
                      placeholder={
                        replyingToComment
                          ? t("language") === "hi"
                            ? `@${replyingToComment.username} को उत्तर दें...`
            ...
            <View style={styles.cgModalContent}>
              ...
              >
                <Text
                  style={[
                    styles.cgItemText,
                    language === "en" && styles.cgItemTextSelected,
                  ]}
                >
                  {t("english")}
                </Text>
              ...
              >
                <Text
                  style={[
                    styles.cgItemText,
                    language === "hi" && styles.cgItemTextSelected,
                  ]}
                >
                  {t("hindi")}
                </Text>
