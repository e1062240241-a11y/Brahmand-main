import { getRamayanChapter, API_URL } from '../../src/services/api';
...
const PARSED_PREFIX = 'parsed:ramayan:';
...
const parseChapter = async (num: number): Promise<any[] | null> => {
  ...
  try {
    ...
    const verses = Array.isArray(data?.verses) ? data.verses.map(normalizeRamayanVerse) : [];
...
const fetchAndStoreRaw = async (num: number): Promise<boolean> => {
  try {
    try {
      const res = await fetch(`${CDN_BASE}/ramayan/chapter-${num}.json`);
