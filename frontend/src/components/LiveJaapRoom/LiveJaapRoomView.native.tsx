...
export default function LiveJaapRoomView() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (AppState.currentState === 'active' && bgPlayer.currentTime !== undefined && bgPlayer.currentTime !== null) {
  ...
  const syncAudio = () => {
    ...
    if (mantraType === 'hanuman') {
      ...
      if (status.isActive) {
        let expected = 0;
        if (mantraType === 'gayatri') {
          expected = (status.elapsedSeconds + 2.0) % 29.276;
        } else if (mantraType === 'shiva') {
          expected = status.elapsedSeconds % 8.48;
        } else if (mantraType === 'krishna') {
          expected = status.elapsedSeconds % 22.77;
        }
        if (expected > 0) {
          bgPlayer.seekTo(expected);
        }
  ...
  useEffect(() => {
    ...
    socketService.connect().then(() => {
      socketService.joinRoom(rName).then(() => {
        syncAudio();
      });
  ...
  return (
    <ImageBackground source={require('../../../assets/images/live_jaap_room_bg.webp')} style={styles.container} resizeMode="cover">
            ...
            <View style={styles.activeRoomContainerNew}>
              ...
              <View style={styles.bottomAreaNew}>
                ...
                <View style={styles.reactionRowNew}>
                  <TouchableOpacity style={styles.reactionBtnNew} onPress={() = accessibilityRole="button" accessibilityLabel="Button"> addReaction('❤️')}>
                     <Text style={styles.reactionEmojiNew}>❤️</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.reactionBtnNew} onPress={() = accessibilityRole="button" accessibilityLabel="Button"> addReaction('🙏')}>
                     <Text style={styles.reactionEmojiNew}>🙏</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.reactionBtnNew} onPress={() = accessibilityRole="button" accessibilityLabel="Button"> addReaction('🔥')}>
                     <Text style={styles.reactionEmojiNew}>🔥</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={styles.reactionBtnNew} onPress={() = accessibilityRole="button" accessibilityLabel="Button"> addReaction('ॐ')}>
                ...
                <View style={styles.controlsBarNew}>
                  ...
                  <TouchableOpacity onPress={() = accessibilityRole="button" accessibilityLabel="Button"> setIsMuted(!isMuted)} style={isMuted ? styles.controlIconBtnNew : styles.volumeMuteBtnNew}>
                    {isMuted ? (
